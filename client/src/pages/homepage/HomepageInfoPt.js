...
const verifyUser = async (code) => {
  try {
    const { data } = await axios.post(`/api/v1/auth/verify-user-pt`, code);
    return data;
  } catch (error) {}
};

function HomepageInfoPt() {
  useEffect(() => {
    setValues({
      firstname: 'Por favor verifique o codigo',
      surname: 'Por favor verifique o codigo',
    });
  }, []);
